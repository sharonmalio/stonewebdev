...
class UsersTable
{
	...
	public function fetchAll($array=FALSE)
	{
		...
		$orderCol = $platform->quoteIdentifier('users_id');
	...
	public function fetchAllUsers(array $columns = NULL, array $joins = NULL, array $literals = NULL, $limit = NULL, $group = NULL, $order = NULL, $buffer = FALSE)
	{
		$platform = $this->getAdapter()->getPlatform();
		...
		$setstmt = $this->getAdapter()->createStatement("set @users_row_num = 0");
		$set = $setstmt->execute();
	...
	public function getUsersObject($id)
	{
		$id  = (int) $id;
		$platform = $this->getAdapter()->getPlatform();
		$table = $platform->quoteIdentifier($this->getTable());
		$Col = $platform->quoteIdentifier('users_id');
	...
	public function saveUsers(Users $users)
	{
		$data = array(
			...
			'account_password'=>$users->account_password,
			);

			$id = (int)$users->users_id;
			if ($id == 0) {
	...
	public function conditionalUpdateUsers($data,array $condition)
	{
	...
	{
		$this->tableGateway->delete(array('users_id' => $id));
	}
	...
	public function getUsersCol($Col)
	{
		$platform = $this->getAdapter()->getPlatform();
		$Col = $platform->quoteIdentifier($Col); 
		$id = $platform->quoteIdentifier('users_id');
		$table = $platform->quoteIdentifier($this->getTable());
	...
	public function getUsersRecordsCount()
	{
		$platform = $this->getAdapter()->getPlatform();
		$table = $platform->quoteIdentifier($this->getTable());
	...
	public function fetchUsersByCol($searchTerm,$WhereCol)
	{
		$platform = $this->getAdapter()->getPlatform();
		$WhereCol = $platform->quoteIdentifier($WhereCol);
		$table = $platform->quoteIdentifier($this->getTable());
