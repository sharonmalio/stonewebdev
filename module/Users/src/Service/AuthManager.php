...
class AuthManager
{
     ...
     */
    public function login($identity_field, $password, $rememberMe)
    {
        ...
        $validator = new \Zend\Validator\EmailAddress(\Zend\Validator\Hostname::ALLOW_DNS);
        if ($validator->isValid($identity_field)) {
            $authAdapter->setEmail($identity_field);
        } else {
            $authAdapter->setUsername($identity_field);
        }
