...
class UserManager
{
     ...
     */
    public function createDefaultUserIfNotExists()
    {
        $user = $this->entityManager->getRepository(User::class)->findOneBy([]);
        if ($user==null) {
            $user = new User();
            $user->setEmail('default@stonehmis.org');
            $user->setFirstName('Default');
            $user->setFirstName('User');
            $user->setDisplayName('Default User');
            $bcrypt = new Bcrypt();
            $passwordHash = $bcrypt->create('default');
            $user->setPassword($passwordHash);
     ...
     * This token is then stored in database and
    ...
    public function generatePasswordResetToken($user)
    {
        ...
        $passwordResetUrl = 'http://' . $httpHost . '/setpassword?token=' . $token . "&email=" . $user->getEmail();
        // Produce HTML of password reset email
        $bodyHtml = $this->viewRenderer->render('users/email/resetpasswordemail', [
        ...
        $mail->setBody($body);
        $mail->setFrom('no-reply@afyaresearch.org', 'Password Reset');
