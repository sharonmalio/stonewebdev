...
class AppointmentsTable
{
	...
	public function fetchAllAppointments(array $columns = NULL, array $joins = NULL, array $literals = NULL, $limit = NULL, $group = NULL, $order = NULL, $buffer = FALSE, $having = NULL)
	{
		$platform = $this->getAdapter()->getPlatform();
		...
		$setstmt = $this->getAdapter()->createStatement("set @appointments_row_num = 0");
		$set = $setstmt->execute();
	...
	public function getAppointmentsObject($id)
	{
		$id  = (int) $id;
		$platform = $this->getAdapter()->getPlatform();
		$table = $platform->quoteIdentifier($this->getTable());
	...
	public function saveAppointments(Appointments $appointments)
	{
		$data = array(
			...
			'appointments_users_id'=>$appointments->appointments_users_id,
			...
			$id = (int)$appointments->appointment_id;
			if ($id == 0) {
	...
	public function updateAppointments($data,$id)
	{
		$this->tableGateway->update($data, array('appointment_id' => $id));
	}

	public function conditionalUpdateAppointments($data,array $condition)
	{
	...
	public function getAppointmentsCol($Col)
	{
		$platform = $this->getAdapter()->getPlatform();
		$Col = $platform->quoteIdentifier($Col); 
	...
	public function getAppointmentsRecordsCount()
	{
		$platform = $this->getAdapter()->getPlatform();
		$table = $platform->quoteIdentifier($this->getTable());
	...
	public function fetchAppointmentsByCol($searchTerm,$WhereCol)
	{
		$platform = $this->getAdapter()->getPlatform();
		$WhereCol = $platform->quoteIdentifier($WhereCol);
		$table = $platform->quoteIdentifier($this->getTable());
