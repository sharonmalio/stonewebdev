...
class ProviderTable
{
	...
	public function fetchAll($array=FALSE)
	{
		...
		$orderCol = $platform->quoteIdentifier('provider_id');
	...
	public function fetchAllProvider(array $columns = NULL, array $joins = NULL, array $literals = NULL, $limit = NULL, $group = NULL, $order = NULL, $buffer = FALSE, $having = NULL)
	{
		$platform = $this->getAdapter()->getPlatform();
		...
		$setstmt = $this->getAdapter()->createStatement("set @provider_row_num = 0");
		$set = $setstmt->execute();
	...
	public function getProviderObject($id)
	{
		$id  = (int) $id;
		$platform = $this->getAdapter()->getPlatform();
		$table = $platform->quoteIdentifier($this->getTable());
		$Col = $platform->quoteIdentifier('provider_id');
	...
	public function saveProvider(Provider $provider)
	{
		$data = array(
			...
			if ($this->getProvider($id)) {
				$this->tableGateway->update($data, array('provider_id' => $id));
			} else {
			throw new RuntimeException(sprintf(
			        'Could not find row with identifier %d',
			        $id
	...
	public function conditionalUpdateProvider($data,array $condition)
	{
	...
	{
		$this->tableGateway->delete(array('provider_id' => $id));
	}
	...
	public function getProviderCol($Col)
	{
		$platform = $this->getAdapter()->getPlatform();
		$Col = $platform->quoteIdentifier($Col); 
		$id = $platform->quoteIdentifier('provider_id');
		$table = $platform->quoteIdentifier($this->getTable());
	...
	public function getProviderRecordsCount()
	{
		$platform = $this->getAdapter()->getPlatform();
		$table = $platform->quoteIdentifier($this->getTable());
	...
	public function fetchProviderByCol($searchTerm,$WhereCol)
	{
		$platform = $this->getAdapter()->getPlatform();
		$WhereCol = $platform->quoteIdentifier($WhereCol);
		$table = $platform->quoteIdentifier($this->getTable());
